...
* [`src/clj/arrival_test_task/`](src/clj/arrival_test_task/): Backend and middleware source files (Clojure,
[Compojure](https://github.com/weavejester/compojure))
* [`src/cljs/arrival_test_task/`](src/cljs/arrival_test_task/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/arrival_test_task/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `arrival-test-task` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/arrival_test_task/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns arrival-test-task.example
...
standalone `arrival-test-task.jar`.
...
```sh
port=2000 java -jar target/arrival-test-task.jar
```
